...
sig_promoters <- function(x, promoters, save_path, save_fasta = FALSE){
  for (i in seq(4, ncol(x), by=2)){
    ...
    weblogo(as.character(data2$promoter), file.out = paste0(save_path, name, "_weblogo.pdf"), open = FALSE)
    if (save_fasta == TRUE){
