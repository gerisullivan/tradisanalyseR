structure_embl <- function(){
  ...
  tag <- str_match(embl_filter, pattern = 'locus_tag.*')
  tag <- tag[!is.na(tag)][1]
  tag <- gsub("locus_tag=", "", tag)
  tag <- gsub("_.*", "", tag)

  locus_tags <- str_extract(embl_filter, paste0(tag, "_[0-9]+"))
  ...
  joined <- myfiles %>% reduce(full_join, by = "locus_tag")
  all_locus <- list(joined, locus_tags) %>% reduce(full_join, by = "locus_tag")

  info <- all_locus[,c(1:3)]
  colnames(info) <- c("locus_tag", "gene", "function")

  filenames <- list.files(pattern = "*.csv") %>% gsub(pattern = ".csv", replace = "")
